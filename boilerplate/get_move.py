...
def get_closest_pellet(ai, map):
    ...
    def to_tupl(pos):
        return pos.x, pos.y, dist(pos.x, pos.y)

    try:
        return min((to_tupl(pos) for pos in map.pellet_positions), key = lambda p : p[2])
    except:
        return 5, 5, 0
...
def get_closest_super_pellet(ai, map):
    ...
    return min((to_tupl(pos) for pos in map.super_pellets_positions), key = lambda p : p[2])
...
def _get_move(ai,map):
    def check_if_blocked(cur_pos, next_pos):
        ...
        while Q:
            ...
            for ngbhr in map.get_neighbours_of(cur):
                ...
                if ngbhr == prev or ngbhr == ai.enemy.pos: continue
                Q.append((ngbhr, cur))
    ...
    # No one is dangerous
    if ai.states.no_danger() or 10 < distance_to_enemy(ai,map):
        ...
        if not has_pellets and not has_spellets:
            ...
            def move_value(move):
                ...
                botx, boty = len(map.content[0]), len(map.content)

                print("HIDING")
                return abs(botx-to_x)+abs(boty-to_y)
        ...
        # Go for super pellet
        if has_spellets and (not has_pellets or spellet_dist < 4 * pellet_dist):
            x, y = spellet_x, spellet_y

        # Go for normal pellet
        elif has_pellets and (not has_spellets or spellet_dist >= 4 * pellet_dist):
        ...
        def move_value(move):
            ...
            cx, cy = len(map.content[0]) / 2, len(map.content) / 2 -5
